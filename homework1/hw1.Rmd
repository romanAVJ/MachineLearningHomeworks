...
clasifica<-function(X,train_df){
  ...
  #Array with delta values for "setosa"
    ...
    delta(test_df[,i], apriori(train_df,column,"setosa"), 
          ...
          , var.covar(train_df,column, "setosa",func_columns))
  }) 
  ...
  #Array with delta values for "virginica"
   virginica <- sapply(1:ncol(test_df),function(i){
    delta(test_df[,i], apriori(train_df,column,"virginica"), 
          ...
          , var.covar(train_df,column, "virginica",func_columns))
  }) 
...
En general, el LDA se desempeñó de manera consistente, independientemente del número de variables que se consideraron para el set de prueba. En los tres casos, los thresholds para **precision**, **recall** y **f1** nunca fueron más bajos que **`r min(min(df[,1]), min(df3[,1]), min(df2[,1]))`**, **`r min(min(df[,2]), min(df3[,2]), min(df2[,2]))`** y **`r min(min(df[,3]), min(df3[,3]), min(df2[,3]))`**,  correspondientemente. Viendo que al utilizar las variables Petal.Length y Petal.Width tenemos una precisión del **`r 1-(4/30)`** que son las que más tienen un valor en la predicción.

# Modelo Multivariado
Para este caso calculamos las funciones Mult_apriori, Mult_media, Mult_var.covar, Mult_delta y Mult_clasifica, que son muy similares al modelo univariado.

## Subtarea 1: Probabilidades a priori de las clases


```{r function_Mult_apriori}
Mult_apriori<-function(train_data_frame, Column_Class, Class){
...
```{r function_Mult_media}
Mult_media<-function(train_data_frame, Column_Class, Class,Column_Ind_Var){
...
```{r}
Mult_var.covar<-function(train_data_frame, Column_Class, Class,Column_Ind_Var){
...
```{r}
Mult_delta<-function(func_test_data, func_apriori, func_mean, func_var){
...
Mult_clasifica<-function(func_test_data, train_data){
  
  func_columns<-c(names(func_test_data))
  Predict_var <- "Species"
  func_Predict_classes <- c("setosa","versicolor","virginica")
  
  setosa <- Mult_delta(func_test_data,Mult_apriori(train_data,Predict_var, "setosa"),Mult_media(train_data,Predict_var, "setosa",func_columns),Mult_var.covar(train_data,Predict_var, "setosa",func_columns))
  versicolor <- Mult_delta(func_test_data,Mult_apriori(train_data,Predict_var, "versicolor"),Mult_media(train_data,Predict_var, "versicolor",func_columns),Mult_var.covar(train_data,Predict_var, "versicolor",func_columns))
  virginica<- Mult_delta(func_test_data,Mult_apriori(train_data,Predict_var, "virginica"),Mult_media(train_data,Predict_var, "virginica",func_columns),Mult_var.covar(train_data,Predict_var, "virginica",func_columns))
...
clas<-Mult_clasifica(test_data, train_data)
clas
```


## Subtarea6: Predicciones y resultados Multivariados
En esta sección aplicamos el LDA al set de entrenamiento (20% de los datos de iris) y lo comparamos con las clasificaciones verdaderas solamente para las variables de Petal.Length Petal.Width. Utilizando el método multivariado.

```{r}
cl_Mult = Mult_clasifica(test_data[3:4], train_data)
cl_Mult
```
Comparamos los resultados originales y las predicciones con una matriz de confusión:

```{r}
table <- table(test_species$Species, cl_Mult$Predict,
               dnn = c("Actual", "Predecido"))
table
```

En este caso, vemos que, cuando incluimos las variables independientes 'Petal.Length','Petal.Width'), el método es capaz de clasificar fácilmente *Setosa*, pero solamente falla en una clasificación, teniendo una estimación de mas del **`r 1-(1/30)`**
